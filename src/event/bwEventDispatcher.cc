...
namespace bWidgets
{
	...
	void bwEventDispatcher::dispatchMouseMovement(bwEvent event)
	{
		...
		if (bwScreenGraph::bwNode* active = context.active)
		{
			if (isDragging())
			{
				bubbleEvent<bwMouseButtonDragEvent&>(event, *active, &bwScreenGraph::bwEventHandler::onMouseDrag, drag_event.value());
			}
		}
		else
		{
			...
			{
				bubbleEvent<bwEvent&>(event, *new_hovered, &bwScreenGraph::bwEventHandler::onMouseMove, event);
			}
	...
	void bwEventDispatcher::dispatchMouseButtonPress(bwMouseButtonEvent& event)
	{
		...
		if (node)
		{
			bubbleEvent<bwMouseButtonEvent&>(event, *node, &bwScreenGraph::bwEventHandler::onMousePress, event);
		}
	...
	void bwEventDispatcher::dispatchMouseButtonRelease(bwMouseButtonEvent& event)
	{
		if (context.active)
		{
			bubbleEvent<bwMouseButtonEvent&>(event, *context.active, &bwScreenGraph::bwEventHandler::onMouseRelease, event);
			...
			if (!isDragging())
			{
				...
				bubbleEvent<bwMouseButtonEvent&>(event, *context.active, &bwScreenGraph::bwEventHandler::onMouseClick, event);
			}
		}
	...
	void bwEventDispatcher::changeContextHovered(bwScreenGraph::bwNode* new_hovered, bwEvent& event)
	{
		...
		if (old_hovered)
		{
			bubbleEvent<bwEvent&>(event, *old_hovered, &bwScreenGraph::bwEventHandler::onMouseLeave, event);
		}

		if (new_hovered)
		{
			bubbleEvent<bwEvent&>(event, *new_hovered, &bwScreenGraph::bwEventHandler::onMouseEnter, event);
		}
