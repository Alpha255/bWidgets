...
namespace bWidgets
{
	...
	{
		using iterator_type = bwScreenGraph::bwNode::ChildList::const_iterator;
		for (auto iter = ++iterator_type(current); iter != (*current)->Parent()->Children()->end(); ++iter)
		{
			if ((*iter)->isVisible())
			{
				return iter->get();
	...
	{
		using iterator_type = bwScreenGraph::bwNode::ChildList::reverse_iterator;
		for (auto iter = iterator_type(current); iter != (*current)->Parent()->Children()->rend(); ++iter)
		{
			if ((*iter)->isVisible())
			{
				return iter->get();
	...
	static void alignNode(bwScreenGraph::bwNode::ChildIterator node_iter, const bwLayoutItem::FlowDirection flow_direction)
	{
	...
	static bool needsMarginAfterNode(const bwScreenGraph::bwNode::ChildList::const_iterator node_iter, const bool align)
	{
	...
	uint32_t bwLayoutItem::countRowColumns(const bwScreenGraph::bwNode::ChildList& children) const
	{
	 ...
	 */
	uint32_t bwLayoutItem::countNeededMargins(const bwScreenGraph::bwNode::ChildList& children) const
	{
	...
	bwPanelLayout::bwPanelLayout() 
		: bwLayoutItem(bwLayoutItem::Type::PANEL, false, FLOW_DIRECTION_VERTICAL)
	{
	}
