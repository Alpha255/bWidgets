...
namespace bWidgets
{
	...
	bwScrollViewHandler::bwScrollViewHandler(bwScrollView& panel) 
		: scrollview(panel)
	{
	}
	...
	{
		return scrollview.isScrollable() && scrollview.scrollbar_node->Rectangle().isCoordinateInside(event.location.x, event.location.y);
	}
	...
	{
		forwardEventToScrollbarIfInside<bwEvent&>(*this, *scrollview.scrollbar_node, event, &bwScreenGraph::bwEventHandler::onMouseMove, event);
