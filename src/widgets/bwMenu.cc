...
void bwMenu::draw(bwStyle& style)
{
    ...
    /* Button rect is limited to the label width — not the full widget/screen
     * width. */
    const bwRectanglePixel btn_rect{
        title_rect.xmin, title_rect.xmin + preferred_label_width, title_rect.ymin, title_rect.ymax
    };
    ...
    if (is_open)
    {
        ...
        painter.drawRoundboxWidgetBase(
            item_hover_style, style, btn_rect, hover_gradient, item_hover_style.corner_radius);
    }
...
/** Draws the dropdown panel with all items (called as an overlay after the main
 * draw pass). */
void bwMenu::drawDropdown(bwStyle& style)
{
    ...
    painter.drawRoundboxWidgetBase(
        base_style, style, dropdown_rect, gradient, base_style.corner_radius);
                      ...
                      bool hovered)
{
    ...
    if (item.type == bwMenuItem::Type::SUBMENU)
    {
        ...
        painter.setActiveColor(can_highlight ? item_hover_style.textColor() :
                                               item_style.decorationColor());
